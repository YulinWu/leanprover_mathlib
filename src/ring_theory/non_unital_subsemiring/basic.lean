...
/- The things in this section need to go elsewhere. -/
section prelim

namespace mul_mem_class

variables {A M : Type*} [has_mul M] [set_like A M] [hA : mul_mem_class A M] (S' : A)
include hA
...
@[to_additive "The natural add monoid hom from an additive subsemigroup of additive semigroup `M`
to `M`."]
...
@[priority 75] /- Prefer subclasses of `non_unital_semiring` over subclasses of
`non_unital_subsemiring_class`. -/
...
/- need to merge `non_unital_comm_semiring` to get this.
...
variables (s : non_unital_subsemiring R)

/-- A non-unital subsemiring contains the semiring's 0. -/
theorem zero_mem : (0 : R) ∈ s := s.zero_mem'

/-- A non-unital subsemiring is closed under multiplication. -/
theorem mul_mem : ∀ {x y : R}, x ∈ s → y ∈ s → x * y ∈ s := s.mul_mem'

/-- A non-unital subsemiring is closed under addition. -/
theorem add_mem : ∀ {x y : R}, x ∈ s → y ∈ s → x + y ∈ s := s.add_mem'

/-- Sum of a list of elements in a `non_unital_subsemiring` is in the `non_unital_subsemiring`. -/
lemma list_sum_mem {l : list R} : (∀x ∈ l, x ∈ s) → l.sum ∈ s :=
s.to_add_submonoid.list_sum_mem

/-- Sum of a multiset of elements in a `non_unital_subsemiring` of a `non_unital_semiring` is
in the `non_unital_add_subsemiring`. -/
lemma multiset_sum_mem (m : multiset R) :
  (∀a ∈ m, a ∈ s) → m.sum ∈ s :=
s.to_add_submonoid.multiset_sum_mem m

/-- Sum of elements in an `non_unital_subsemiring` of an `non_unital_semiring` indexed by a `finset`
is in the `non_unital_add_subsemiring`. -/
lemma sum_mem (s : non_unital_subsemiring R)
